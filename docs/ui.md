...
│ ░  │   [リポジトリ名 ▲]        [最終更新日 ▼]  │  ░ │
│ ░  │   ┌─ スクロール領域 ──────────────────┐  │  ░ │
│ ░  │   │ owner/repo-1  最終更新: YYYY-MM-DD [削除]│ │  ░ │
│ ░  │   │ owner/repo-2  最終更新: YYYY-MM-DD [削除]│ │  ░ │
│ ░  │   └───────────────────────────────────┘  │  ░ │
│ ░  └──────────────────────────────────────────┘  ░ │
└─────────────────────────────────────────────────────┘
```
...
| リポジトリ件数表示 | 「リポジトリ」見出しの横に登録件数 `(N)` を表示。最終更新日の取得中でも登録数（`getRepos()` ベース）を正しく表示する |
| リポジトリ追加エラー | 「リポジトリが見つかりません、またはアクセス権限がありません」（赤） |
| リポジトリ一覧 Loading | 「読み込み中...」表示 |
